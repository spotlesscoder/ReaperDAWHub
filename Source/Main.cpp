...
std::shared_ptr<action_entry>
add_action(std::string name, std::string id, toggle_state togst,
...
void toggleBrowserWindow(action_entry &) {
  ...
  if (g_browser_wnd == nullptr) {
    g_browser_wnd =
        new Window("The Inter Webs", 700, 400, true, Colours::black);
    // This call order is important, the window should not be set visible
    // before adding it into the Reaper window hierarchy
    // Currently this only works for Windows, OS-X needs some really annoying
    // special handling not implemented yet
#ifdef WIN32
    g_browser_wnd->addToDesktop(g_browser_wnd->getDesktopWindowStyleFlags(),
                                GetMainHwnd());
#else
...
REAPER_PLUGIN_ENTRYPOINT(REAPER_PLUGIN_HINSTANCE hInstance,
  ...
  if (rec != nullptr) {
    ...
    add_action("JUCE test : Show browser", "JUCETEST_SHOW_BROWSER",
               CannotToggle, [](action_entry &ae) { toggleBrowserWindow(ae); });
    add_action("JUCE test : Show login", "JUCETEST_SHOW_LOGIN", CannotToggle,
               [](action_entry &ae2) { toggleLoginWindow(ae2); });
