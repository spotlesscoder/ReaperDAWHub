...
REAPERAPI_DEF //==============================================
    ...
    // seconds unless qn is set.
        ...
        MediaItem *(*CreateNewMIDIItemInProj)(MediaTrack *track,
...
REAPERAPI_DEF //==============================================
    ...
    // GetAudioAccessorHash, GetAudioAccessorStartTime, GetAudioAccessorEndTime,
...
REAPERAPI_DEF //==============================================
    ...
    // CreateTakeAudioAccessor, CreateTrackAudioAccessor, GetAudioAccessorHash,
...
REAPERAPI_DEF //==============================================
              // EnumerateFiles
    // List the files in the "path" directory. Returns NULL (or empty string, in
    // Lua) when all files have been listed. See EnumerateSubdirectories
...
REAPERAPI_DEF //==============================================
    ...
    // string, in Lua) when all subdirectories have been listed. See
    // EnumerateFiles
...
REAPERAPI_DEF //==============================================
    ...
    // is the seond derivative, dddVdS is the third derivative. See
...
REAPERAPI_DEF //==============================================
    ...
    // DestroyAudioAccessor, GetAudioAccessorHash, GetAudioAccessorStartTime,
    // GetAudioAccessorSamples.
...
REAPERAPI_DEF //==============================================
    ...
    // Get a short hash string (128 chars or less) that will change only if the
    // underlying samples change.  See CreateTakeAudioAccessor,
...
REAPERAPI_DEF //==============================================
    ...
    // CreateTrackAudioAccessor, DestroyAudioAccessor, GetAudioAccessorHash,
    // GetAudioAccessorStartTime, GetAudioAccessorEndTime.//
    //
...
REAPERAPI_DEF //==============================================
    ...
    // See CreateTakeAudioAccessor, CreateTrackAudioAccessor,
    // DestroyAudioAccessor, GetAudioAccessorHash, GetAudioAccessorEndTime,
    // GetAudioAccessorSamples.
...
REAPERAPI_DEF //==============================================
    ...
    // position. See GetEnvelopePoint, SetEnvelopePoint, Envelope_Evaluate.
...
REAPERAPI_DEF //==============================================
    ...
    // track, 1 means track 1, etc. itemnumber and fxnumber are zero-based. See
    // GetLastTouchedFX.
...
REAPERAPI_DEF //==============================================
    ...
    // for fade change, 4 for item move
...
REAPERAPI_DEF //==============================================
    ...
    // tracknumber==0 means the master track, 1 means track 1, etc. fxnumber and
    // paramnumber are zero-based. See GetFocusedFX.
...
REAPERAPI_DEF //==============================================
    ...
    // C_BEATATTACHMODE : char * to one char of beat attached mode, -1=def,
    // 0=time, 1=allbeats, 2=beatsosonly C_LOCK : char * to one char of lock
    // flags (&1 is locked, currently) D_VOL : double * of item volume (volume
    // bar) D_POSITION : double * of item position (seconds) D_LENGTH : double *
    // of item length (seconds) D_SNAPOFFSET : double * of item snap offset
    // (seconds) D_FADEINLEN : double * of item fade in length (manual, seconds)
    ...
    // D_FADEINLEN_AUTO : double * of item autofade in length (seconds, -1 for
    // no autofade set) D_FADEOUTLEN_AUTO : double * of item autofade out length
    // (seconds, -1 for no autofade set) C_FADEINSHAPE : int * to fadein shape,
    // 0=linear, ... C_FADEOUTSHAPE : int * to fadeout shape I_GROUPID : int *
    // to group ID (0 = no group) I_LASTY : int * to last y position in track
    // (readonly) I_LASTH : int * to last height in track (readonly)
    ...
    // be used (though will store the color anyway). I_CURTAKE : int * to active
    // take IP_ITEMNUMBER : int, item number within the track (read-only,
    // returns the item number directly) F_FREEMODE_Y : float * to free mode y
    // position (0..1) F_FREEMODE_H : float * to free mode height (0..1)
...
REAPERAPI_DEF //==============================================
    ...
    // D_PITCH : double *, take pitch adjust (in semitones, 0.0=normal, +12 =
    // one octave up, etc) B_PPITCH, bool *, preserve pitch when changing rate
    // I_CHANMODE, int *, channel mode (0=normal, 1=revstereo, 2=downmix, 3=l,
    // 4=r) I_PITCHMODE, int *, pitch shifter mode, -1=proj default, otherwise
    // high word=shifter low word = parameter I_CUSTOMCOLOR : int *, custom
    // color, OS dependent color|0x100000 (i.e. ColorToNative(r,g,b)|0x100000).
    // If you do not |0x100000, then it will not be used (though will store the
    // color anyway). IP_TAKENUMBER : int, take number within the item
...
REAPERAPI_DEF //==============================================
    ...
    // IP_TRACKNUMBER : int : track number (returns zero if not found, -1 for
    // master track) (read-only, returns the int directly) I_SOLO : int * :
    // 0=not soloed, 1=solo, 2=soloed in place I_FXEN : int * : 0=fx bypassed,
    // nonzero = fx active I_RECARM : int * : 0=not record armed, 1=record armed
    // I_RECINPUT : int * : record input. <0 = no input, 0..n = mono hardware
    // input, 512+n = rearoute input, 1024 set for stereo input pair. 4096 set
    // for MIDI input, if set, then low 5 bits represent channel (0=all,
    // 1-16=only chan), then next 5 bits represent physical input (63=all,
    // 62=VKB) I_RECMODE : int * : record mode (0=input, 1=stereo out, 2=none,
    // 3=stereo out w/latcomp, 4=midi output, 5=mono out, 6=mono out w/ lat
    // comp, 7=midi overdub, 8=midi replace I_RECMON : int * : record monitor
    // (0=off, 1=normal, 2=not when playing (tapestyle)) I_RECMONITEMS : int * :
    // monitor items while recording (0=off, 1=on) I_AUTOMODE : int * : track
    // automation mode (0=trim/off, 1=read, 2=touch, 3=write, 4=latch) I_NCHAN :
    // int * : number of track channels, must be 2-64, even I_SELECTED : int * :
    // track selected? 0 or 1 I_WNDH : int * : current TCP window height
    // (Read-only) I_FOLDERDEPTH : int * : folder depth change (0=normal,
    // 1=track is a folder parent, -1=track is the last in the innermost folder,
    // -2=track is the last in the innermost and next-innermost folders, etc
    // I_FOLDERCOMPACT : int * : folder compacting (only valid on folders),
    // 0=normal, 1=small, 2=tiny children I_MIDIHWOUT : int * : track midi
    // hardware output index (<0 for disabled, low 5 bits are which channels
    // (0=all, 1-16), next 5 bits are output device index (0-31)) I_PERFFLAGS :
    // int * : track perf flags (&1=no media buffering, &2=no anticipative FX)
    // I_CUSTOMCOLOR : int * : custom color, OS dependent color|0x100000 (i.e.
    // ColorToNative(r,g,b)|0x100000). If you do not |0x100000, then it will not
    // be used (though will store the color anyway). I_HEIGHTOVERRIDE : int * :
    // custom height override for TCP window. 0 for none, otherwise size in
    // pixels D_VOL : double * : trim volume of track (0 (-inf)..1 (+0dB) .. 2
    // (+6dB) etc ..) D_PAN : double * : trim pan of track (-1..1) D_WIDTH :
    // double * : width of track (-1..1) D_DUALPANL : double * : dualpan
    // position 1 (-1..1), only if I_PANMODE==6 D_DUALPANR : double * : dualpan
    // position 2 (-1..1), only if I_PANMODE==6 I_PANMODE : int * : pan mode (0
    // = classic 3.x, 3=new balance, 5=stereo pan, 6 = dual pan) D_PANLAW :
    // double * : pan law of track. <0 for project default, 1.0 for +0dB, etc
    // P_ENV : read only, returns TrackEnvelope *, setNewValue=<VOLENV, <PANENV,
    // etc B_SHOWINMIXER : bool * : show track panel in mixer -- do not use on
    // master B_SHOWINTCP : bool * : show track panel in tcp -- do not use on
    // master B_MAINSEND : bool * : track sends audio to parent C_MAINSEND_OFFS
    // : char * : track send to parent channel offset B_FREEMODE : bool * :
    // track free-mode enabled (requires UpdateTimeline() after changing etc)
    // C_BEATATTACHMODE : char * : char * to one char of beat attached mode,
    // -1=def, 0=time, 1=allbeats, 2=beatsposonly F_MCP_FXSEND_SCALE : float * :
    // scale of fx+send area in MCP (0.0=smallest allowed, 1=max allowed)
    // F_MCP_SENDRGN_SCALE : float * : scale of send area as proportion of the
    // fx+send total area (0=min allow, 1=max)
...
REAPERAPI_DEF //==============================================
    ...
    // get the currently selected track envelope, returns 0 if no envelope is
    // selected
...
REAPERAPI_DEF //==============================================
    ...
    // setNewValue to update) B_MUTE : bool * to muted state B_LOOPSRC : bool *
    // to loop source B_ALLTAKESPLAY : bool * to all takes play B_UISEL : bool *
    // to ui selected C_BEATATTACHMODE : char * to one char of beat attached
    // mode, -1=def, 0=time, 1=allbeats, 2=beatsosonly C_LOCK : char * to one
    // char of lock flags (&1 is locked, currently) D_VOL : double * of item
    // volume (volume bar) D_POSITION : double * of item position (seconds)
...
REAPERAPI_DEF //==============================================
    ...
    // retrieve the old source, set the new, THEN delete the old. GUID : GUID *
    // : 16-byte GUID, can query or update P_NAME : char * to take name
    // D_STARTOFFS : double *, start offset in take of item
    // D_VOL : double *, take volume
    // D_PAN : double *, take pan
    // D_PANLAW : double *, take pan law (-1.0=default, 0.5=-6dB, 1.0=+0dB, etc)
...
REAPERAPI_DEF //==============================================
    ...
    // GUID : GUID * : 16-byte GUID, can query or update (do not use on master
    // though) P_NAME : char * : track name (on master returns NULL) P_ICON :
    // const char * : track icon (full filename, or relative to
...
REAPERAPI_DEF //==============================================
    ...
    // Group can be one of: VOLUME_MASTER VOLUME_SLAVE VOLUME_VCA_MASTER
    // VOLUME_VCA_SLAVE
    // PAN_MASTER
    // PAN_SLAVE
    // WIDTH_MASTER
...
REAPERAPI_DEF //==============================================
    ...
    // characteristics. 0=MIDI colormap image file, 1=MIDI bank/program select
    // file, 2=MIDI text string file, 3=MIDI note mapping file. If fn != NULL, a
    // new track MIDI storage file will be set; otherwise the existing track
    // MIDI storage file will be returned.
...
REAPERAPI_DEF //==============================================
    ...
    // source track, only applies for sends/recvs P_ENV : read only, returns
    // TrackEnvelope *, setNewValue=<VOLENV, <PANENV, etc
    //
    // B_MUTE : returns bool *
    ...
    // I_SENDMODE : returns int *, 0=post-fader, 1=pre-fx, 2=post-fx
    // (deprecated), 3=post-fx I_AUTOMODE : returns int * : automation mode
    // (-1=use track automode, 0=trim/off, 1=read, 2=touch, 3=write, 4=latch)
    // I_SRCCHAN : returns int *, index,&1024=mono, -1 for none
    // I_DSTCHAN : returns int *, index, &1024=mono, otherwise stereo pair,
    // hwout:&512=rearoute I_MIDIFLAGS : returns int *, low 5 bits=source
    // channel 0=all, 1-16, next 5 bits=dest channel, 0=orig, 1-16=chan See
    // CreateTrackSend, RemoveTrackSend.
...
REAPERAPI_DEF //==============================================
    ...
    bool (*GetTrackMIDILyrics)(MediaTrack *track, int flag, char *bufWant16384,
                               int *bufWant16384_sz);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    // Get note name. pitch 128 for CC0 name, 129 for CC1 name, etc. See
...
REAPERAPI_DEF //==============================================
    ...
    // To increase text field width, add an extra caption field, and specify
    // extrawidth=xyz
...
REAPERAPI_DEF //==============================================
    ...
    // GetEnvelopePoint, SetEnvelopePoint, GetEnvelopeScalingMode.
...
REAPERAPI_DEF //==============================================
    ...
    LICE_IBitmap *(*LICE_LoadPNGFromResource)(HINSTANCE hInst, int resid,
                                              LICE_IBitmap *bmp);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    // if you pass a .RTrackTemplate file then it adds that to the project
    // instead.
...
REAPERAPI_DEF //==============================================
    ...
    // previous event, flag: &1=selected &2=muted, msglen: byte length of msg
    // (usually 3), msg: the MIDI message. For tick intervals longer than a 32
    // bit word can represent, zero-length meta events may be placed between
    // valid events. See MIDI_SetAllEvts.
...
REAPERAPI_DEF //==============================================
    ...
    // not include bounding F0..F7), 1-7:MIDI text event types.
...
REAPERAPI_DEF //==============================================
    ...
    // should not include bounding F0..F7), 1-7:MIDI text event types.
...
REAPERAPI_DEF //==============================================
    ...
    // (usually 3), msg: the MIDI message. For tick intervals longer than a 32
    // bit word can represent, zero-length meta events may be placed between
    // valid events. See MIDI_GetAllEvts.
...
REAPERAPI_DEF //==============================================
    ...
    // F0..F7), 1-7:MIDI text event types. set noSort if setting multiple
    // events, then call MIDI_Sort when done.
...
REAPERAPI_DEF //==============================================
    ...
    // other extensions or plug-ins, use register("API_myfunction",funcaddress),
    // and "-API_myfunction" to remove. Other extensions then use
    // GetFunc("myfunction") to get the function pointer. REAPER will also
    // export the function address to ReaScript, so your extension could supply
    // a Python module that provides a wrapper called RPR_myfunction(..).
    // register("APIdef_myfunction",defstring) will include your function
    // declaration and help in the auto-generated REAPER API header and
    // ReaScript documentation. defstring is four null-separated fields: return
    // type, argument types, argument names, and help. Example: double
    // myfunction(char* str, int flag) would have
    // defstring="double\0char*,int\0str,flag\0help text for myfunction"
    //
    // another thing you can register is "hookcommand", which you pass a
    // callback:
    //  NON_API: bool runCommand(int command, int flag);
    //           register("hookcommand",runCommand);
    // which returns TRUE to eat (process) the command.
    // flag is usually 0 but can sometimes have useful info depending on the
    // message. note: it's OK to call Main_OnCommand() within your runCommand,
    // however you MUST check for recursion if doing so! in fact, any use of
    // this hook should benefit from a simple reentrancy test...
    //
    ...
    //  NON_API: bool onAction(KbdSectionInfo *sec, int command, int val, int
    //  valhw, int relmode, HWND hwnd);
    ...
    // register with "command_id", parameter is a unique string with only A-Z,
    // 0-9, returns command ID (or 0 if not supported/out of actions)
    //
    // register("command_id_lookup", unique_string) will look up the integer ID
    // of the named action without registering the string if it doesn't already
    // exist.
...
REAPERAPI_DEF //==============================================
    ...
    void (*screenset_registerNew)(char *id, screensetCallbackFunc callbackFunc,
                                  void *param);
#endif
...
REAPERAPI_DEF //==============================================
    ...
    // Envelope_SortPoints when done. See GetEnvelopePoint, InsertEnvelopePoint,
    // GetEnvelopeScalingMode.
...
REAPERAPI_DEF //==============================================
    ...
    // or the MIDI-as-control input queue. mode=0 for VKB, 1 for control
    // (actions map etc), 2 for VKB-on-current-channel.
...
REAPERAPI_DEF //==============================================
    ...
    // value for instantiate to always create a new effect, 0 to only query the
    // first instance of an effect, or a positive value to add an instance if
    // one is not found.
...
REAPERAPI_DEF //==============================================
    ...
    // a positive value to add an instance if one is not found.
