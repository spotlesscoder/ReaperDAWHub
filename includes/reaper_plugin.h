...
typedef struct reaper_plugin_info_t {
  ...
  // this is the API that plug-ins register most things, be it keyboard
  // shortcuts, project importers, etc. typically you register things on load of
  // the DLL, for example:

  // static pcmsink_register_t myreg={ ... };
  // rec->Register("pcmsink",&myreg);

  // then on plug-in unload (or if you wish to remove it for some reason), you
  // should do: rec->Register("-pcmsink",&myreg); the "-" prefix is supported
  // for most registration types.
...
public:
  virtual void
  AddItem(MIDI_event_t *evt) = 0; // puts the item in the correct place
...
typedef struct {
  ...
  int overwritemode; // 0=overdub, 1=replace,
                     ...
                     // 65536+(16 bit mask) = replace just these channels
                     // (touch-replace)
  ...
  unsigned int
      *overwrite_actives; // [16][4]; only used when overwritemode is >0
...
public:
  ...
  virtual int LoadState(char *firstline,
...
#define PCM_SOURCE_EXT_INLINEEDITOR                                            \
  0x100 // parm1 = (void *)(INT_PTR)message, parm2/parm3 = parms
        // messages: 0 = query if editor is available/supported. returns <0 if
        // supported but unavailable, >0 if available, 0=if not supported
        //  WM_CREATE to create the editor instance, WM_DESTROY to destroy it
        //  (nonzero if success) WM_ERASEBKGND / WM_PAINT / WM_NCPAINT (3
        //  stages) for drawing, parm2 = LICE_IBitmap *, parm3 =
        //  (REAPER_inline_positioninfo*) WM_LBUTTON*, WM_RBUTTON*,
        //  WM_MOUSEMOVE, WM_MOUSEWHEEL
        // parm2=rsvd, parm3= REAPER_inline_positioninfo) --
        // return REAPER_INLINE_* flags
        // WM_SETCURSOR gets parm3=REAPER_inline_positioninfo*, should return
        // HCURSOR WM_KEYDOWN gets parm3=REAPER_inline_positioninfo* with
        // extraParms[0] to MSG*
...
#define REAPER_INLINEFLAG_WANTOVERLAYEDCONTROLS                                \
  0x4000 // only valid as a ret for msg 0, to have fades/etc still drawn over
         // like normal
...
#define PCM_SOURCE_EXT_OPENEDITOR                                              \
  0x10001 // parm1=hwnd, parm2=track idx, parm3=item description
#define PCM_SOURCE_EXT_GETEDITORSTRING                                         \
  0x10002 // parm1=index (0 or 1), returns desc
// 0x10003 is deprecated
#define PCM_SOURCE_EXT_SETITEMCONTEXT                                          \
  0x10004 // parm1=pointer to item handle, parm2=pointer to take handle
...
#define PCM_SOURCE_EXT_GETMETADATA                                             \
  0x10007 // parm1=pointer to name string, parm2=pointer to buffer,
          ...
          // "DESC", "ORIG", "ORIGREF", "DATE", "TIME", "UMID", "CODINGHISTORY"
          // (i.e. BWF)
#define PCM_SOURCE_EXT_SETASSECONDARYSOURCE                                    \
  0x10008 // parm1=optional pointer to src (same subtype as receiver), if
          ...
          // editor open to attach to, parm2=trackidx, parm3=itemname
...
#define PCM_SOURCE_EXT_ENUMCUES                                                \
  0x90005 // parm1=(int) index of cue to get, parm2=REAPER_cue **. returns
          // 0/sets parm2 to NULL when out of cues
...
#define PCM_SOURCE_EXT_ENUMSLICES                                              \
  0x90006 // parm1=(int*) index of slice to get, parm2=REAPER_slice* (pointing
          // to caller's existing slice struct). if parm2 passed in zero,
          // returns the number of slices. returns 0 if no slices or out of
          // slices.
...
#define PCM_SOURCE_EXT_SETPREVIEWTEMPO                                         \
  0x90008 // parm1=(double*)bpm, only meaningful for slice-based source media
...
typedef struct {
  ...
  const char *(*EnumFileExtensions)(
      int i, char **descptr); // call increasing i until returns a string, if
...
typedef struct accelerator_register_t {
  ...
  // -666 to force it to the main window's accel table (with the exception of
  // ESC)
...
typedef LRESULT (*screensetCallbackFunc)(int action, char *id, void *param,
                                         int param2);
...
#define UNDO_STATE_TRACKCFG                                                    \
  1 // has track/master vol/pan/routing, ALL envelopes (matser included)
