// ReaperDAWHub.Playground.cpp : Defines the entry point for the console
// application.
//
...
template <typename R> bool isReady(std::future<R> const &f) {
...
int main() {
  ...
  std::future<std::vector<Project>> f =
      std::async(std::launch::async, &ServiceClient::getAvailableProjects, &sc);
